...
class PhotoAdapter(
    ...
    inner class ViewHolder(
        ...
        override fun bind(item: UIPhoto) {
            ...
            val width =
                ((binding.root.resources.displayMetrics.widthPixels / 2) - (scale * 24)).toInt()
            val height =
                ((width) / ((item.width.toFloat() / item.height.toFloat()))).toInt()

            binding.ivPhoto.layoutParams.height = height
